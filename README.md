

---

## 🧑‍🎓 STUDENT DETAILS
Name      : Jaideep Singh  
URN       : 2302721  
Title     : 6-Month Training Daily Diary  
Domain    : Cyber Security 
Date      : 21-06-2025 to 23-07-2025 at CYBERCORE Technologies

---

## 📌 TRAINING OVERVIEW
This section documents the detailed day-wise training activities for the
initial four weeks of my 6-month industrial training in Cyber Security.
The activities include theoretical learning and practical exposure
to system security, networking, and ethical cyber practices.

---

# 📅 WEEK-WISE & DAY-WISE TRAINING DETAILS

---

## 🟦 WEEK 1 – Introduction to Cyber Security

### Day 1
• Introduction to industrial training  
• Overview of cyber security domain  
• Importance of cyber security in modern systems  

### Day 2
• Understanding information security  
• CIA Triad: Confidentiality, Integrity, Availability  
• Real-life examples of data breaches  

### Day 3
• Introduction to cyber threats  
• Types of attacks: malware, hacking, phishing  
• Basic security terminology  

### Day 4
• Ethical hacking overview  
• Difference between ethical hacking and illegal hacking  
• Legal boundaries in cyber security  

### Day 5
• Introduction to cyber security tools  
• Overview of Kali Linux  
• Importance of secure environments  

---

## 🟦 WEEK 2 – Linux Fundamentals

### Day 1
• Installation of Kali Linux in virtual machine  
• Understanding virtualization concepts  

### Day 2
• Introduction to Linux file system  
• Understanding directories and paths  

### Day 3
• Practicing basic Linux commands  
• Commands: ls, cd, mkdir, pwd  

### Day 4
• File permissions and ownership  
• chmod command and its importance  

### Day 5
• Importance of Linux in cyber security  
• Daily revision and practice  

---

## 🟦 WEEK 3 – Networking Basics

### Day 1
• Introduction to computer networks  
• Importance of networking in cyber security  

### Day 2
• Understanding IP addresses (IPv4)  
• Private and public IP concepts  

### Day 3
• Introduction to ports and protocols  
• Common ports and services  

### Day 4
• TCP vs UDP protocols  
• Data transmission basics  

### Day 5
• OSI and TCP/IP models  
• Layer-wise understanding  

---

## 🟦 WEEK 4 – Url Safety work

### Day 1
• Introduction to url safety  
• How it works

### Day 2
• Writing Scripts for url in python  
• testing the scripts

### Day 3
• Updation to url safety checker  
• adding API's for real time Checking

### Day 4
• Final Testing and adding data
• Re-Testing and Deployment  

### Day 5
• Using at Real time Scanning with api  
• Weekly revision and documentation  

---

## ✅ LEARNING OUTCOME (After 4 Weeks)
• Strong understanding of cyber security fundamentals  
• Basic Linux operating skills  
• Clear networking concepts  
• Understanding of web and client–server architecture  

---

## 🎯 VIVA ONE-LINE EXPLANATION
This phase of training focused on building strong fundamentals in cyber
security, Linux systems, networking, and web architecture.
